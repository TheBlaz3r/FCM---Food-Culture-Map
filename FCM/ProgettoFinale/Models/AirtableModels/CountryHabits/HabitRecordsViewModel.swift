...
class RecordsViewModel2 {
    var records: [HabitRecord] = [] // A list to store fetched records from Airtable.
    
    private let CountryHabitsTableURL = "https://api.airtable.com/v0/appqEjD69spsgV8hE/tblYDSnW9LhmxP1WY"
    ...
    func fetchRecords() {
        guard let url = URL(string: CountryHabitsTableURL) else { return } // Ensure the URL is valid.
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let data = data { // Ensure we have data to work with.
                do {
                    let response = try JSONDecoder().decode(AirtableResponse2.self, from: data) // Decode the JSON into our model.
